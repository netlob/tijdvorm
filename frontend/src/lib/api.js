...
//   VITE_API_HOST=http://m4.netlob:8000
export const API_HOST = stripTrailingSlash(
  (import.meta.env.VITE_API_HOST &&
    String(import.meta.env.VITE_API_HOST).trim()) ||
    "http://m4.netlob:8000",
);
...
export async function apiGetOverride() {
  ...
  if (!res.ok)
    throw new Error((await asText(res)) || "Failed to load override");
...
export async function apiSetEnabled(filename, enabled) {
  const res = await fetch(
    `${API_BASE}/images/${encodeURIComponent(filename)}/enabled`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export async function apiSetExplicit(filename, explicit) {
  const res = await fetch(
    `${API_BASE}/images/${encodeURIComponent(filename)}/explicit`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export async function apiDeleteImage(filename) {
  const res = await fetch(
    `${API_BASE}/images/${encodeURIComponent(filename)}`,
    { method: "DELETE" },
  );
...
export async function apiGetSettings() {
  ...
  if (!res.ok)
    throw new Error((await asText(res)) || "Failed to load settings");
  return await res.json();
}
...
export async function apiGetLivePreview() {
  ...
  if (!res.ok)
    throw new Error((await asText(res)) || "Failed to load live preview");
  const data = await res.json();
...
export async function apiSetPriority(filename, priority) {
  const res = await fetch(
    `${API_BASE}/images/${encodeURIComponent(filename)}/priority`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
