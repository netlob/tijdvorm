...
python debug/airplay/debug_pyatv.py

# Or run specific commands
python debug/airplay/debug_pyatv.py scan          # Scan for devices
python debug/airplay/debug_pyatv.py test          # Run comprehensive tests
python debug/airplay/debug_pyatv.py stream <url>  # Test specific URL
python debug/airplay/debug_pyatv.py diagnose      # iPhone compatibility diagnostics
```
...
- **airplayer**: `npm install airplayer` - Full client implementation ⭐83
  - ✅ **Finds Samsung TVs** successfully
  - ❌ **Same streaming error** as pyatv: "Unexpected response to PTTH/1.0 Upgrade request"
  - 🎯 **Confirms the issue is Samsung TV compatibility, not client library**
...
python debug/airplay/debug_pyatv.py stream http://localhost:8000/api/render/doorbell.jpg
```

## Testing Alternative Libraries

#### Test Results Summary
- **pyatv (Python)**: ❌ "RTSP/1.0 method PUT failed with code 501: Not Implemented"
- **airplayer (Node.js)**: ❌ "Unexpected response to PTTH/1.0 Upgrade request"
- **DLNA (Python)**: ✅ Works reliably with Samsung TVs

**Conclusion**: Both Python and Node.js AirPlay clients exhibit the same fundamental compatibility issue with Samsung Frame TVs. The issue appears to be with Samsung's AirPlay protocol implementation, not the client libraries.

### Quick Node.js Test
```bash
# Navigate to airplay debug directory
cd debug/airplay
...
pip install python-airplay

# Test with your Samsung TV
python debug/airplay/debug_pyatv.py  # Use our comprehensive test script
```
