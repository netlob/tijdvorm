...
async def main():
    ...
    tv = SamsungTVAsyncArt(host='10.0.1.111', port=8002, token_file='tv_token.txt')
    ...
    if supported:
        try:
            ...
            # all_content_ids = ["MY_F{}".format(i) for i in range(0, 27000)]
            # # all_content_ids = ["MY_F{}".format(i) for i in range(10000, 19000)]
            # # all_content_ids = ["MY_F{}".format(i) for i in range(19000, 25001)]
            # batch_size = 50
            
            # for i in range(0, len(all_content_ids), batch_size):
            #     batch = all_content_ids[i:i + batch_size]
            #     try:
            #         await tv.delete_list(batch)
            #         logging.info('deleted batch from tv: {} - {}'.format(batch[0], batch[-1]))
            #         await asyncio.sleep(1)
            #     except Exception as e:
            #         logging.warning('failed to delete batch {} - {}: {}'.format(batch[0], batch[-1], e))
